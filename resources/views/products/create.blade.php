...
{{--        @endif--}}
        ...
        <form action="{{ route('products.store') }}" method="POST" class="max-w-2xl mx-auto border rounded-lg p-6">
            @csrf
            <div>
                <label for="title" class="font-bold">Product title</label>
                <input type="text" name="title" id="title" class="block border w-full py-2 px-4 rounded-lg mt-2">
                @error('title')
            ...
            <div class="mt-4">
                <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg">Create product
                </button>
            </div>
        </form>
    </main>
